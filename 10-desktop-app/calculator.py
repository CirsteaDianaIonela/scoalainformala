...
window.resizable(False, False)  #sa nu poata fi modificata fereastra, sa o maresc, sa o micsorez
...
def egalitate():  #atunci cand apasam pe egal ne calculeaza expresia
    try:
...
impartit = Button(frame_butoane, text='/', width=18, height=3, bg='#FFA500', cursor='hand2', command=lambda: click('/'))
impartit.grid(row=0, column=3) #ca sa nu mai apara erorare cu grid scriu ca aici
...
inmultire = Button(frame_butoane, text='*', width=18, height=3, bg='#FAA500', cursor="hand2", command=lambda: click('*')).grid(row=1, column=3)
...
scadere = Button(frame_butoane, text='-', width=18, height=3, bg='#FAA500', cursor="hand2", command=lambda: click('-')).grid(row=2, column=3)
...
adunare = Button(frame_butoane, text='+', width=18, height=3, bg='#FAA500', cursor="hand2", command=lambda: click('+')).grid(row=3, column=3)
...
egal = Button(frame_butoane, text='=', width=18, height=3, bg='#eee', cursor="hand2", command=lambda: egalitate()).grid(row=4, column=3)

window.mainloop()  #ne ajuta sa rulam, creeaza fereastra
#sa realizam un executor-> de instalat pip install -U pyinstaller
#pt rulare avem comanda pyinstaller --onefile calculator.py
#va creea cateva fisiere, deschidem cu open si apare inconita calculator in folder dist
